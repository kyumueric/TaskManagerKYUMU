            ...
            <form action="{{ route('admin.statuses.update', $status->id) }}" method="POST">
                ...
                <div class="box-body">
                    ...
                    <div class="form-group @error('color') has-error @enderror">
                        ...
                        <input type="color" name="color" class="form-control" value="{{ old('color', $status->color) }}" required style="height: 38px; width: 70px; padding: 3px;">
                        @error('color')
                            <span class="help-block">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="form-group">
                        <label>Preview:</label>
                        <span class="badge" id="colorPreview" style="background-color: {{ old('color', $status->color) }}"> Sample Status </span>
                    </div>
                </div>
